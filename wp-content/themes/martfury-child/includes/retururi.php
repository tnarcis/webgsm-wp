...
add_action('woocommerce_account_retururi_endpoint', function() {
        ...
        if(empty($order_id) || empty($product_id) || empty($motiv) || empty($tip_retur) || $qty_retur < 1) {
            ...
            if($qty_retur > $qty_available) {
                ...
                if($days_since_delivery > 14) {
                    ...
                    // Verifică dacă e PJ și restricționează tipul de retur
                    ...
                    // Pentru PJ, doar "defect" și "altul" sunt permise
    ...
    jQuery(document).ready(function($) {
        // Gestionare afișare nume fișiere pentru retur
            ...
            $.ajax({
                ...
                success: function(response) {
                    if(response.success) {
                        ...
                        // Restaurează valoarea dacă e validă
...
add_action('wp_ajax_get_order_products_retur', function() {
    ...
    // Verifică tipul de facturare folosind același meta key ca în plugin-ul checkout
    $customer_type = $order->get_meta('_customer_type');
    if(empty($customer_type)) {
        $customer_type = $order->get_meta('_billing_customer_type');
    }
    ...
    <table class="form-table">
        ...
        <tr>
            ...
            <td>
                ...
                <br><a href="<?php echo admin_url('admin-ajax.php?action=download_storno_pdf&retur_id=' . $post->ID); ?>" class="button" target="_blank">📄 Descarca PDF</a>
            </td>
        </tr>
        <?php endif; ?>
        <tr>
